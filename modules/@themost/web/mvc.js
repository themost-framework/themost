...
function HttpContentResult(content) {
    HttpContentResult.super_.bind(this)();
    this.data = content;
    this.contentType = 'text/html';
    this.contentEncoding = 'utf8';
}
...
function HttpEmptyResult() {
    HttpEmptyResult.super_.bind(this)();
}
...
{
    HttpJsonResult.super_.bind(this)();
...
{
    HttpJavascriptResult.super_.bind(this)();
...
{
    HttpXmlResult.super_.bind(this)();
...
function HttpRedirectResult(url) {
    HttpRedirectResult.super_.bind(this)();
    this.url = url;
}
...
function HttpFileResult(physicalPath, fileName) {
    HttpFileResult.super_.bind(this)();
...
{
    HttpViewResult.super_.bind(this)();
    this.name = name;
...
function HttpViewEngine(context) {
    HttpViewEngine.super_.bind(this)();
}
...
function HttpViewContext(context) {
    HttpViewContext.super_.bind(this)();
    /**
...
HttpViewContext.HtmlViewHelper = function($view)
{
    ...
    antiforgery: function() {
        //create token
        var context = $view.context,  value = context.application.getStrategy(EncryptionStrategy).encrypt(JSON.stringify({ id: Math.floor(Math.random() * 1000000), url:context.request.url, date:new Date() }));
        //try to set cookie
