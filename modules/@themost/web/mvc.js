...
var HttpXmlResult = exports.HttpXmlResult = function (_HttpResult5) {
    ...
    function HttpXmlResult(data) {
        ...
        if ((typeof data === 'undefined' ? 'undefined' : _typeof(data)) === 'object') _this5.data = _mostXml2.default.serialize(data, { item: 'Item' }).outerXML();else _this5.data = data;
        return _this5;
    }
    ...
    _createClass(HttpFileResult, [{
        ...
        value: function execute(context, callback) {
            ...
            var physicalPath = this.physicalPath,
                ...
                app = require('./index');
            _fs2.default.exists(physicalPath, function (exists) {
                if (!exists) {
                    callback(new app.HttpNotFoundError());
                } else {
                    try {
                        _fs2.default.stat(physicalPath, function (err, stats) {
                            if (err) {
                                callback(err);
                            } else {
                                if (!stats.isFile()) {
                                    ...
                                    var _ret = function () {
                                        ...
                                        //get file extension
                                        var extensionName = _path2.default.extname(fileName || physicalPath);
                                        //get MIME collection
                                        ...
                                        if (_lodash._.isNil(contentType)) {
                                            ...
                                            //create read stream
                                            var source = _fs2.default.createReadStream(physicalPath);
                                            //add Content-Disposition: attachment; filename="<file name.ext>"
                                            context.response.writeHead(200, {
                                                'Content-Type': contentType + (contentEncoding ? ';charset=' + contentEncoding : ''),
                                                'ETag': responseETag
...
function queryAbsoluteViewPath(search, controller, view, extension, callback) {
    var result = _path2.default.resolve(search, _util2.default.format('%s/%s.html.%s', controller, view, extension));
    _fs2.default.exists(result, function (exists) {
        ...
        result = _path2.default.resolve(search, _util2.default.format('%s/%s.html.%s', capitalizedController, view, extension));
        _fs2.default.exists(result, function (exists) {
            if (exists) return callback(null, result);
            callback();
        });
...
function isAbsolute(p) {
    ...
    return _path2.default.normalize(p + '/') === _path2.default.normalize(_path2.default.resolve(p) + '/');
}
...
var HttpViewResult = exports.HttpViewResult = function (_HttpResult8) {
    ...
    _createClass(HttpViewResult, [{
        ...
        value: function execute(context, callback) {
            ...
            var viewPath = void 0,
                viewEngine = void 0;
            _async2.default.eachSeries(context.application.config.engines, function (engine, cb) {
                if (viewPath) {
                    cb();return;
                }
                if (routePath && isAbsolute(routePath)) {
                    ...
                    (function () {
                        ...
                        if (routePath) {
                            searchViewName = _path2.default.join(routePath, viewName);
                        }
    ...
    _createClass(HttpController, [{
        ...
        value: function result(data) {
            if (this.context) {
                ...
                if (typeof fn !== 'function') throw new _errors.HttpError(400, 'Not implemented.');
