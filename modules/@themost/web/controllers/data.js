...
{
    HttpDataController.super_.bind(this)(context);
    var model_;
    var self = this;
...
HttpDataController.prototype.filter = function (callback) {
    ...
    if (!_.isObject(self.model)) {
        return callback(new Error('Model is of the wrong type or undefined.'));
    }
         ...
         function (err, q) {
            try {
                if (err) {
                    ...
                    if (expand) {
                        var resolver = require("@themost/data/expand-resolver");
