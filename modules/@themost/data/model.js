...
var Q = _interopRequireDefault(_q2).default;
...
var DataValidatorListener = _validators.DataValidatorListener;
...
var DataModel = exports.DataModel = function (_SequentialEventEmitt) {
     ...
     */
    function DataModel(obj, context) {
            ...
            try {
                parser.parse(filter, function (err, query) {
                    if (err) {
                        ...
                        var _q = new DataQueryable(self);
                        _q.query.$where = query;
                        if ($joinExpressions.length > 0) _q.query.$expand = $joinExpressions;
                        //prepare
                        _q.query.prepare();
                        ...
                        if ((typeof params === 'undefined' ? 'undefined' : _typeof(params)) === 'object') {
                            ...
                            if (typeof select === 'string') {
                                _q.select.apply(_q, select.split(',').map(function (x) {
                                    return x.replace(/^\s+|\s+$/g, '');
                                }));
                            }
                            //apply group by fields
                            if (typeof groupBy === 'string') {
                                _q.groupBy.apply(_q, groupBy.split(',').map(function (x) {
                                    return x.replace(/^\s+|\s+$/g, '');
                                }));
                            }
                            ...
                            if (orderBy) {
                                orderBy.split(',').map(function (x) {
                                    ...
                                    if (/\s+desc$/i.test(x)) {
                                        _q.orderByDescending(x.replace(/\s+desc$/i, ''));
                                    } else if (/\s+asc/i.test(x)) {
                                        _q.orderBy(x.replace(/\s+asc/i, ''));
                                    } else {
                                        _q.orderBy(x);
                                    }
                                });
