                            ...
                            if (requestMask === PermissionMask.Create) {
                                ...
                                model.attributes.forEach(function (x) {
                                    ...
                                    if (obj.hasOwnProperty(name)) {
                                        var _mapping = model.inferMapping(name);
                                        if (_.isNil(_mapping)) {
                                            field = {};
                                            field[x.name] = { $value: obj[name] };
                                            fields.push(field);
                                        } else if (_mapping.associationType === 'association' && _mapping.childModel === model.name) {
                                            if (_typeof(obj[name]) === 'object') {
                                                //set associated key value (e.g. primary key value)
                                                field = {};
                                                field[x.name] = { $value: obj[name][_mapping.parentField] };
                                                fields.push(field);
                                            } else {
                                                //set raw value
                                                field = {};
                    ...
                    async.eachSeries(privileges, function (item, cb) {
                        ...
                        try {
                            if (item.type === 'global') {
                                ...
                                if (_.isNil(expr)) expr = QueryExpression.create();
